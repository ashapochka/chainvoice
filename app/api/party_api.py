...
async def get_many(
    ...
    return await party_service.get_many(db, user, offset, limit)
...
async def get_one(
    ...
    return await party_service.get_one_by_uid(db, user, uid)
...
async def create_one(
    ...
    result = await party_service.create(db, user, obj)
...
async def update_one(
        *, db=Depends(get_db),
        user: UserInDb = Depends(get_current_active_user),
        uid: str, obj: PartyUpdate
):
    await party_service.update_by_uid(db, user, uid, obj)
...
async def delete_one(
    ...
    await party_service.delete_by_uid(db, user, uid)
